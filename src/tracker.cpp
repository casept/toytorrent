...
TrackerCommunicator::TrackerCommunicator(std::string announce_url, std::uint32_t our_port, PeerID our_peer_id, std::string infohash) {
    m_data_downloaded = {0};
    m_data_uploaded = {0};
    m_data_left = {0};
...
BEncodeObject get_object_from_dict_or_throw(std::string key, std::map<std::string, BEncodeObject> dict, std::string throw_msg) {
...
std::vector<Peer> bencode_peer_list_to_peers(std::vector<BEncodeObject> list) {
    ...
    for (BEncodeObject const& peer_entry: list) {
        ...
        auto peer_id_obj = get_object_from_dict_or_throw("peer id", peer_dict, "Tracker violated protocol: all peers must have peer ID");
        auto peer_ip_obj = get_object_from_dict_or_throw("ip", peer_dict, "Tracker violated protocol: all peers must have IP address");
        auto peer_port_obj = get_object_from_dict_or_throw("port", peer_dict, "Tracker violated protocol: all peers must have port");
...
std::tuple<std::vector<Peer>, std::int64_t> TrackerCommunicator::send_started() {
    return send_to_tracker("started");
}

std::tuple<std::vector<Peer>, std::int64_t> TrackerCommunicator::send_stopped() {
    return send_to_tracker("stopped");
}

std::tuple<std::vector<Peer>, std::int64_t> TrackerCommunicator::send_update() {
    return send_to_tracker("empty");
}